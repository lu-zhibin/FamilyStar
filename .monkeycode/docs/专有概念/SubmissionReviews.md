...
任务 6.1 为单人打卡与协作提交接入家长人工审核，任务 6.2 增加按家庭规则执行的超时自动通过，任务 6.3 将单人 `APPROVED` 状态与积分发放纳入同一事务，任务 6.4 在单人和协作审核通过路径接入动态 Streak，并在协作全员通过时统一发分。实现位于 `apps/api/src/check-ins/review-*`，覆盖审核决策、历史查询、家庭隔离、有界候选批次、幂等重试和并发保护。
...
阶段 6 覆盖家长人工审核、超时自动通过、统一历史、动态 Streak、单人/协作通过后的积分事务、等级派生与权益读取，以及核心属性与竞态测试；阶段 11 将超时批处理接入独立 Worker。

## 验证

领域测试覆盖家长权限、拒绝原因、幂等重试、48 小时边界、0 禁用、家庭级超时、批次上限、目标锁与重复执行。Prisma 仓储测试覆盖两种目标的候选读取、latest attempt 复查、条件更新、`TIMEOUT` 历史、家长并发竞争，以及单人/协作的家长与超时发分衔接；HTTP 测试覆盖人工与超时历史输出。任务 6.6 验证并发审核仅一个请求进入仓储，以及 1 至 72 小时超时的精确边界；owner-lock 会等待完整仓储写入后释放。阶段 6 完成时全仓 56 个测试文件、345 个测试全部通过。
