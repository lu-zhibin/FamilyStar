...
产品补全任务 5.1 建立成长记录持久化基础，任务 5.2 接入家长时间线读取和手动记录 CRUD，任务 5.3 接入审核通过事件和幂等投影，任务 5.4 接入双端页面，任务 5.5 补齐迁移、家庭隔离、来源幂等、历史快照、稳定分页和组件状态属性测试。
...
成长记录直接保存标题、正文、发生日期和积分，媒体通过独立业务关联引用原始 COS 对象。任务后续编辑、停用或归档不会改写这些展示字段。消费者使用批准事件中的任务名称、提交正文、家庭自然日期、最终积分和有序媒体 ID 创建记录；仓储在返回自动记录时用标题快照生成任务展示名，同时保留任务 ID 供筛选。
...
- 家长 `/records` 展示家庭成长时间线，支持按孩子、任务、类型、开始日期和结束日期筛选，并通过稳定 cursor 继续加载。
- 家长可创建、编辑和软删除 `NOTE`、`MILESTONE`，可关联孩子、可选任务及最多十个 READY 图片或视频；自动 `CHECK_IN` 记录只读。
- 孩子 `/child/records` 读取 `/check-ins/me/history`，展示当前孩子的单人和协作提交、审核状态、积分、正文和 READY 媒体。
- 两端汇总当前时间线媒体 ID，去重后按每批最多五十个调用 `POST /api/v1/media/access-urls`。媒体签名失败时保留文字记录并提供独立重试。
- 首屏、筛选空集合、读取失败、后续分页失败和筛选选项失败均有独立状态；分页失败保留已经加载的记录。
...
2026-08-06 任务 5.1 至 5.5 已通过以下验证：
...
任务 5.2 的仓储、服务和路由聚焦回归共 3 个文件、14 项测试通过；权限联合回归共 45 项通过。任务 5.3 的事件发布、幂等消费者和事务回归共 3 个文件、30 项测试通过。API 全量回归共 98 个文件、638 项通过。任务 5.4 的双端门户、时间线视图、查询、分页和媒体 helper 聚焦回归共 4 个文件、44 项通过，Web 全量回归共 14 个文件、127 项通过。任务 5.5 的迁移、CRUD、消费者、快照、分页和组件聚焦回归共 5 个文件、26 项通过，全量单元回归共 114 个文件、812 项通过。全工作区类型检查、Lint、Prisma Schema、迁移历史、API 与 Web 生产构建和任务文件格式检查通过。
