...
- 取消和采纳均在 `Serializable` 事务中完成。采纳使用家庭范围、活动状态和未删除条件执行原子转换；并发竞争失败会回滚同事务创建的奖励并返回冲突。
...
Prisma 仓储测试覆盖跨家庭采纳在创建奖励前失败，以及并发采纳只允许一个活动状态转换成功。失败事务由积分事务 writer 整体回滚，避免留下未关联奖励。
