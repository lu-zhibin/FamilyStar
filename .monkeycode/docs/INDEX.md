...
- MVP 开发决策：已确认
- 实施任务：Phase 1 基线 12 个阶段，已完成真实运行缺陷修复任务 13、14
- 测试策略：全部测试任务纳入必做范围
- 源码实现：阶段 1 至阶段 12 已完成；核心闭环和自动化质量门禁已通过
- 当前迭代：家长端审核页已接入家庭真实待审队列、稳定幂等写入与服务端权威重拉
- 最近更新：2026-08-02
...
| 文档 | 用途 | 状态 |
|---|---|---|
| `ARCHITECTURE.md` | 当前系统结构、依赖和运行边界 | 已同步至 Phase 1 任务 14 |
| `INTERFACES.md` | 当前 HTTP、类型、数据模型和工作区接口 | 已同步至 Phase 1 任务 14 |
| `DEVELOPER_GUIDE.md` | 环境、命令、工作流和验证方式 | 已同步至 Phase 1 任务 14 |
| `模块/API.md` | Hono API 组合、领域适配器、迁移和验证入口 | 已同步至阶段 7 |
| `模块/Web.md` | 统一登录入口、家长端与孩子端路由、真实数据状态、组件与响应式交互 | 已同步至 Phase 1 任务 13 |
...
| `专有概念/SubmissionReviews.md` | 家庭待审队列、人工与超时审核、历史、幂等与发分衔接 | 已同步至 Phase 1 任务 14 |
