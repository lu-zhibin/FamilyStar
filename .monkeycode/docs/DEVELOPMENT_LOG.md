...
- 分支：`dev`
- 状态：初次修复引入家长端顶部留白回归，已由 `224d59f` 恢复并重新验收
...
- 孩子端 `ChildShell` 的主内容容器仅使用 `page-shell`，双端公共页面起始位置因此产生差异。
...
- 纠正提交 `224d59f` 恢复家长端原有 `py-7 mobile:py-5`，即桌面端 28px、移动端 20px 顶部页边距；回归断言同步锁定该响应式类。
...
- 纠正提交 `224d59f` 已发布到开发环境并通过真实页面验收，部署结果见 `FS-DEPLOY-DEV-008`。
