...
| 字段 | 类型 | 约束 |
|---|---|---|
| `familyCode` | `String` | `@unique`、10 位大写字母和数字、数据库列 `family_code` |

迁移为已有家庭从 UUID 生成稳定初始码，为新家庭使用加密随机源生成 10 位码。数据库唯一索引提供最终冲突保护。
...
- 单元测试覆盖家庭码格式、生成器和服务层家庭解析。
- Prisma 契约测试覆盖字段、唯一索引和迁移约束。
